...
- Status: v1.8 implementation, hosted desktop validation, deterministic packaging, and authorized source publication complete; v1.6 is retained only as the compatibility checkpoint
...
| MS-055 | 6 | **Desktop release candidate.** Windows/Linux/macOS packages build, audit, install, export, and document the exact supported scope. | MS-045; hosted macOS runner; publication authorization for release step. | Platform binaries/dependencies; clean-install/export logs; notices/checksums; upstream-sync report; GNT-04 PASS. | Hosted desktop builds, audits, runtime fixtures, and deterministic packages pass; visible authoring and native-export gates remain open |
...
- **Frozen references:** exact v1.8/source/dependency lock (`VERIFIED`); Redot Windows/Linux/macOS identities and required layouts (`VERIFIED`); `.gdextension` manifest (`VERIFIED`); MIT source license and CC BY 4.0 logo/demo licenses (`VERIFIED`); hosted workflow run `31476931577` at `1470048` (`PASS`).
...
| RSK-006 | Windows/Linux/macOS toolchains or native outputs diverge. | Incomplete desktop release. | Same source/API locks, clean platform CI, manifest and dependency audit. Owner: Codex. | Resolved for the hosted desktop build/audit/runtime matrix by run `31476931577`. |
| RSK-007 | Complete 1.8 feature parity is now required. | A partial backport would not satisfy the authorized product scope. | Merge the full tag, adapt its engine dependencies, and add direct regression coverage. Owner: Codex. | Resolved; exact v1.8 source is published and its later feature groups are directly covered. |
